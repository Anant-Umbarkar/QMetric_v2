...
// ── Shared sub-components (outside ResultPage to avoid remount on re-render) ──
...
const Card = ({ title, children, accent = 'blue' }) => (
  <div className="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-6 mb-6 overflow-hidden relative">
    <div className={`absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r ${
      accent === 'blue'   ? 'from-blue-400 to-indigo-400' :
...
const ResultPage = () => {
  ...
  useEffect(() => {
    ...
    else { setError('Authorization token is required'); setLoading(false); }
  }, []);
  ...
  const fetchData = async (authToken) => {
    try {
      ...
      if (!response.ok) {
        ...
        else if (response.status === 403) throw new Error('Forbidden: Insufficient permissions');
        else throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
      const result = await response.json();
      if (result.success && result.data) setData(result.data);
      else throw new Error('Invalid response format or unsuccessful request');
    } catch (err) {
  ...
  const downloadPDF = async () => {
    try {
      ...
      // ── CO-PO data from backend (correct paths) ──
      // CO_PO_Map is stored at sequence[0].CO_PO_Map (set by upload route)
      // PO_Coverage and CO_PO_Coverage come from Collected Data (set by evaluate.js)
      const coPOMap      = sequence[0]?.CO_PO_Map || {};
      const poCoverage   = collectedData?.PO_Coverage || {};
      const coPOCoverage = collectedData?.CO_PO_Coverage || {};
      const coPOPenalty  = collectedData?.CO_PO_Penalty ?? null;  // null = not applicable
      // PO Achievement = complement of penalty (100 - penalty%)
      const poAchievement = coPOPenalty !== null ? parseFloat((100 - coPOPenalty).toFixed(2)) : null;

      // Derive PO keys from CO_PO_Map for table headers
      const coKeysPDF = Object.keys(coPOMap);
      const poKeysPDF = coKeysPDF.length > 0
        ? [...new Set(Object.values(coPOMap).flatMap(m => Object.keys(m)))]
        : [];
      ...
      // ── CO-PO Mapping section (strength table: 1/2/3) ──
      const coPOMappingSection = poKeysPDF.length > 0 ? `
        <div class="section">
          <div class="section-title">CO-PO Mapping</div>
          <table>
            <thead>
              <tr>
                <th>CO</th>
                ${poKeysPDF.map(po => `<th class="text-center">${po}</th>`).join('')}
              </tr>
            </thead>
            <tbody>
              ${coKeysPDF.map(co => {
                const mapping = coPOMap[co] || {};
                return `<tr>
                  <td><strong>${co}</strong></td>
                  ${poKeysPDF.map(po => {
                    const val = mapping[po];
                    const display = (!val || val === '') ? '–' : val;
                    const color = val === 3 ? '#16a34a' : val === 2 ? '#2563eb' : val === 1 ? '#d97706' : '#9ca3af';
                    return `<td class="text-center" style="color:${color};font-weight:600">${display}</td>`;
                  }).join('')}
                </tr>`;
              }).join('')}
            </tbody>
          ...
          <p style="font-size:9pt;color:#6b7280;margin-top:8px">
            <span style="color:#16a34a;font-weight:600">3</span> = High &nbsp;
            <span style="color:#2563eb;font-weight:600">2</span> = Medium &nbsp;
            <span style="color:#d97706;font-weight:600">1</span> = Low &nbsp;
            <span style="color:#9ca3af">–</span> = No relation
          </p>
        </div>` : '';

      // ── CO-PO Analysis section (penalty, PO coverage, CO-PO coverage) ──
      const coPOAnalysisSection = poKeysPDF.length > 0 ? `
        <div class="section">
          <div class="section-title">CO-PO Analysis</div>

          ${poAchievement !== null ? `
          <div style="display:flex;gap:24px;margin-bottom:24px">
            <div style="flex:1;padding:16px 20px;border:1px solid #e5e5e5;text-align:center">
              <div style="font-size:9pt;text-transform:uppercase;letter-spacing:.5px;color:#666;margin-bottom:6px">CO-PO Penalty</div>
              <div style="font-size:28pt;font-weight:300;color:#ef4444">${coPOPenalty.toFixed(2)}%</div>
            </div>
            <div style="flex:1;padding:16px 20px;border:1px solid #e5e5e5;text-align:center">
              <div style="font-size:9pt;text-transform:uppercase;letter-spacing:.5px;color:#666;margin-bottom:6px">PO Achievement</div>
              <div style="font-size:28pt;font-weight:300;color:${poAchievement >= 80 ? '#16a34a' : poAchievement >= 60 ? '#2563eb' : poAchievement >= 40 ? '#ca8a04' : '#ef4444'}">${poAchievement.toFixed(2)}%</div>
            </div>
          </div>` : ''}

          <p style="font-size:10pt;font-weight:600;margin-bottom:8px">PO Coverage</p>
          <table>
            <thead>
              <tr>
                ${poKeysPDF.map(po => `<th class="text-center">${po}</th>`).join('')}
            ...
            <tbody>
              <tr>
                ${poKeysPDF.map(po => {
                  const val = poCoverage[po] ?? 0;
                  const color = val >= 80 ? '#16a34a' : val >= 60 ? '#2563eb' : val >= 40 ? '#ca8a04' : '#ef4444';
                  return `<td class="text-center" style="color:${color};font-weight:600">${val.toFixed(1)}%</td>`;
                }).join('')}
              </tr>
            </tbody>
          </table>

          <p style="font-size:10pt;font-weight:600;margin:20px 0 8px">CO-PO Mapping Coverage</p>
          <table>
            <thead>
              <tr>
                <th>CO</th>
            ...
            <tbody>
              ${coKeysPDF.map(co => {
                ...
                return `<tr>
                  <td><strong>${co}</strong></td>
                  ${poKeysPDF.map(po => {
                    const val = coverageRow[po];
                    if (val === null || val === undefined) return `<td class="text-center" style="color:#d1d5db">–</td>`;
                    const color = val >= 80 ? '#16a34a' : val >= 60 ? '#2563eb' : val >= 40 ? '#ca8a04' : '#ef4444';
                    return `<td class="text-center" style="color:${color};font-weight:600">${val.toFixed(1)}%</td>`;
                  }).join('')}
                </tr>`;
          ...
          <p style="font-size:9pt;color:#6b7280;margin-top:8px">Coverage % = how much of each CO-PO mapping's maximum potential is actually achieved. – = no mapping defined.</p>
        </div>` : '';

      const coRows = Object.keys(sequence[0]?.COs || {}).map(co => {
        const d = sequence[0].COs[co];
        return `<tr><td class="text-center">${co}</td><td class="text-center">${d.weight || 0}%</td><td class="text-center">${d.blooms?.[0] || 'N/A'}</td></tr>`;
      }).join('');

      const moduleRows = Object.keys(sequence[0]?.ModuleHours || {}).map(mod =>
        `<tr><td class="text-center">${mod}</td><td class="text-center">${sequence[0].ModuleHours[mod] || 0}</td></tr>`
      ).join('');

      const bloomLevelMapRows = Object.keys(blommLevelMap).map(level =>
        `<tr><td>${level}</td><td class="text-center">${blommLevelMap[level]}</td></tr>`
      ).join('');
      ...
      const bloomDataRows = Object.keys(bloomsData).map(level => {
        ...
        return `<tr>
          ...
          <td class="text-center">${bd.level}</td>
          <td class="text-center">${(bd.weights || 0).toFixed(1)}%</td>
          <td class="text-center">${(bd.marks || 0).toFixed(1)}%</td>
          <td class="text-center ${variance < 0 ? 'negative' : 'positive'}">${variance > 0 ? '+' : ''}${variance.toFixed(1)}%</td>
          <td class="text-center">${bd.No_Of_Questions || 0}</td>
        </tr>`;
      }).join('');

      const moduleAnalysisRows = moduleData.map((mod, i) => {
        const variance = (mod.actual || 0) - (mod.expected || 0);
        return `<tr>
          <td class="text-center">Module ${i + 1}</td>
          <td class="text-center">${(mod.expected || 0).toFixed(1)}%</td>
          <td class="text-center">${(mod.actual || 0).toFixed(1)}%</td>
          <td class="text-center ${variance < 0 ? 'negative' : 'positive'}">${variance > 0 ? '+' : ''}${variance.toFixed(1)}%</td>
        </tr>`;
      }).join('');

      const coAnalysisRows = Object.keys(coData).map(co =>
        `<tr><td class="text-center">CO${co}</td><td class="text-center">${(coData[co] || 0).toFixed(1)}%</td><td class="text-center positive">Complete</td></tr>`
      ).join('');

      const questionRows = questionRecommendations.map((rec, i) => `
        <tr>
          ...
          <td class="text-center ${rec.qScore === 1 ? 'status-match' : rec.qScore === 2 ? 'status-higher' : 'status-lower'}">${rec.qScore}</td>
          <td class="text-center ${rec.remark === 'Matches Expected Blooms Level' ? 'status-match' : rec.remark === 'Higher than Expected Blooms Level' ? 'status-higher' : 'status-lower'}">${rec.remark || 'No remarks'}</td>
        </tr>`).join('');
      ...
      const htmlContent = `<!DOCTYPE html><html lang="en"><head>
        ...
        <style>
          *{margin:0;padding:0;box-sizing:border-box}
          body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI','Helvetica Neue',Arial,sans-serif;line-height:1.6;color:#1a1a1a;background:#fff;padding:40px 60px;font-size:11pt;position:relative}
          body::before{content:'QMetric';position:fixed;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-45deg);font-size:120pt;font-weight:700;color:rgba(0,0,0,.03);z-index:-1;white-space:nowrap;pointer-events:none}
          .report-container{max-width:1100px;margin:0 auto}
          .header{border-bottom:2px solid #1a1a1a;padding-bottom:30px;margin-bottom:50px}
          .header h1{font-size:28pt;font-weight:600;color:#1a1a1a;margin-bottom:8px;letter-spacing:-.5px}
          .header-subtitle{font-size:11pt;color:#666;margin-bottom:20px}
          .header-meta{display:flex;justify-content:space-between;font-size:9pt;color:#666;margin-top:20px}
          .section{margin-bottom:50px}
          .section-title{font-size:14pt;font-weight:600;color:#1a1a1a;margin-bottom:20px;letter-spacing:-.3px}
          .info-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px 40px;margin-bottom:30px}
          .info-item{border-bottom:1px solid #e5e5e5;padding-bottom:8px}
          .info-label{font-size:9pt;color:#666;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px}
          .info-value{font-size:11pt;color:#1a1a1a;font-weight:500}
          table{width:100%;border-collapse:collapse;margin:20px 0;font-size:10pt}
          table thead{border-bottom:2px solid #1a1a1a}
          table th{padding:12px 8px;text-align:left;font-weight:600;font-size:9pt;text-transform:uppercase;letter-spacing:.5px;color:#1a1a1a}
          table td{padding:10px 8px;border-bottom:1px solid #f0f0f0;color:#1a1a1a}
          ...
          .score-section{text-align:center;padding:50px 0;border-top:2px solid #1a1a1a;border-bottom:2px solid #1a1a1a;margin:50px 0}
          .score-label{font-size:10pt;text-transform:uppercase;letter-spacing:1px;color:#666;margin-bottom:15px}
          .score-value{font-size:60pt;font-weight:300;color:#1a1a1a;line-height:1;letter-spacing:-2px}
          .stats-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:30px;margin:40px 0}
          .stat-card{text-align:center;padding:20px;border:1px solid #e5e5e5}
          .stat-value{font-size:32pt;font-weight:300;color:#1a1a1a;line-height:1;margin-bottom:8px}
          .stat-label{font-size:9pt;color:#666;text-transform:uppercase;letter-spacing:.5px}
          .analysis-box{background:#fafafa;padding:25px;margin:30px 0;border-left:3px solid #1a1a1a}
          .analysis-box h4{font-size:11pt;font-weight:600;margin-bottom:12px;color:#1a1a1a}
          .analysis-box p{margin:8px 0;font-size:10pt;color:#1a1a1a;line-height:1.7}
          .footer{margin-top:60px;padding-top:30px;border-top:1px solid #e5e5e5;text-align:center;color:#666;font-size:9pt}
          .divider{height:1px;background:#e5e5e5;margin:40px 0}
          @media print{body{padding:20px}.section{margin-bottom:30px;page-break-inside:auto}.section-title{page-break-after:avoid}table{page-break-inside:auto}table tr{page-break-inside:avoid}.score-section{padding:30px 0;margin:30px 0;page-break-inside:avoid}.stats-grid{margin:20px 0;page-break-inside:avoid}.divider{margin:20px 0;page-break-after:avoid}}
        </style></head><body>
        <div class="report-container">
          ...
          <div class="section">
            ...
            <div class="info-grid">
              <div class="info-item"><div class="info-label">Institution</div><div class="info-value">${data['College Name'] || 'N/A'}</div></div>
              <div class="info-item"><div class="info-label">Department</div><div class="info-value">${data['Branch'] || 'N/A'}</div></div>
              <div class="info-item"><div class="info-label">Course</div><div class="info-value">${data['Course Name'] || 'N/A'}</div></div>
              <div class="info-item"><div class="info-label">Code</div><div class="info-value">${data['Course Code'] || 'N/A'}</div></div>
              <div class="info-item"><div class="info-label">Instructor</div><div class="info-value">${data['Course Teacher'] || 'N/A'}</div></div>
              <div class="info-item"><div class="info-label">Academic Period</div><div class="info-value">Year ${data['Year Of Study'] || 'N/A'}, Sem ${data['Semester'] || 'N/A'}</div></div>
            </div>
          </div>
          ...
          <div class="score-section">
            ...
            <div style="margin-top:30px;display:inline-block;padding:20px 40px;border-radius:8px;${
              finalScore >= 80 ? 'background:#f0fdf4;border:2px solid #86efac;' :
              finalScore >= 60 ? 'background:#eff6ff;border:2px solid #93c5fd;' :
              finalScore >= 40 ? 'background:#fefce8;border:2px solid #fde047;' :
                                 'background:#fef2f2;border:2px solid #fca5a5;'}">
              <div style="font-size:24pt;font-weight:700;margin-bottom:8px;${
                finalScore >= 80 ? 'color:#16a34a;' :
                finalScore >= 60 ? 'color:#2563eb;' :
                finalScore >= 40 ? 'color:#ca8a04;' : 'color:#dc2626;'}">
          ...
          <div class="section">
            ...
            <div class="stats-grid">
              ...
              <div class="stat-card"><div class="stat-value">${matchingQuestions}</div><div class="stat-label">Aligned Questions</div></div>
              <div class="stat-card"><div class="stat-value">${higherQuestions}</div><div class="stat-label">Higher Level</div></div>
              <div class="stat-card"><div class="stat-value">${lowerQuestions}</div><div class="stat-label">Lower Level</div></div>
            </div>
          </div>
          ...
          <div class="section">
            ...
            <table><thead><tr><th class="text-center">Outcome</th><th class="text-center">Weight</th><th class="text-center">Target Level</th></tr></thead>
          ...
          <div class="section">
            ...
            <table><thead><tr><th class="text-center">Module</th><th class="text-center">Hours</th></tr></thead>
          ...
          <div class="section">
            ...
            <table><thead><tr><th>Cognitive Level</th><th class="text-center">Value</th></tr></thead>
            <tbody>${bloomLevelMapRows}</tbody></table>
          </div>

          <div class="section">
            <div class="section-title">Detailed Question-wise Analysis</div>
            <table><thead><tr>
              <th class="text-center">#</th><th>Question</th><th class="text-center">Marks</th>
              <th class="text-center">CO</th><th class="text-center">Type</th><th class="text-center">Module</th>
              <th class="text-center">Bloom's Verbs</th><th class="text-center">Level</th>
              <th class="text-center">Highest Verb</th><th class="text-center">Status</th>
            </tr></thead>
            <tbody>${questionData.map((q, i) => `
              <tr>
                <td class="text-center">${i + 1}</td>
                <td>${q.Question || 'N/A'}</td>
                <td class="text-center">${q.Marks || 0}</td>
                <td class="text-center">${q.CO || 'N/A'}</td>
                <td class="text-center">${q['QT'] || 'N/A'}</td>
                <td class="text-center">${q.Module || 'N/A'}</td>
                <td class="text-center">${q["Bloom's Verbs"] || 'N/A'}</td>
                <td class="text-center">${q["Bloom's Taxonomy Level"] || 'N/A'}</td>
                <td class="text-center">${q["Bloom's Highest Verb"] || 'N/A'}</td>
                <td class="text-center ${q.Remark === 'Matches Expected Blooms Level' ? 'status-match' : q.Remark === 'Higher than Expected Blooms Level' ? 'status-higher' : 'status-lower'}">${q.Remark || 'No remarks'}</td>
              </tr>`).join('')}
            </tbody></table>
          </div>
          ...
          <div class="section">
            ...
            <table><thead><tr><th class="text-center">Module</th><th class="text-center">Expected</th><th class="text-center">Actual</th><th class="text-center">Variance</th></tr></thead>
            <tbody>${moduleAnalysisRows}</tbody></table>
          </div>

          <div class="section">
            <div class="section-title">Bloom's Taxonomy Analysis</div>
            <table><thead><tr><th>Level</th><th class="text-center">#</th><th class="text-center">Expected</th><th class="text-center">Actual</th><th class="text-center">Variance</th><th class="text-center">Questions</th></tr></thead>
            <tbody>${bloomDataRows}</tbody></table>
          </div>

          <div class="section">
            <div class="section-title">Course Outcome Coverage</div>
            <table><thead><tr><th class="text-center">Outcome</th><th class="text-center">Coverage</th><th class="text-center">Status</th></tr></thead>
            <tbody>${coAnalysisRows}</tbody></table>
          </div>
          ...
          <div class="section">
            ...
            <table><thead><tr>
              <th class="text-center">#</th><th>Question</th><th class="text-center">Marks</th>
              <th class="text-center">CO</th><th class="text-center">Extracted Verb</th>
              <th class="text-center">Highest Verb</th><th class="text-center">Q-Score</th><th class="text-center">Remark</th>
            </tr></thead>
          ...
          <div class="section">
            ...
            <table style="table-layout:fixed"><thead><tr><th class="text-center">CO</th><th class="text-center">Actual</th><th class="text-center">Expected</th><th class="text-center">Variance</th><th>Recommendation</th></tr></thead>
            <tbody>${coRecommendationRows}</tbody></table>
          </div>` : ''}

          ${moduleRecommendations.length > 0 ? `
          <div class="section">
            <div class="section-title">Module Recommendations</div>
            <table style="table-layout:fixed"><thead><tr><th class="text-center">Module</th><th class="text-center">Actual</th><th class="text-center">Expected</th><th class="text-center">Variance</th><th>Recommendation</th></tr></thead>
            <tbody>${moduleRecommendationRows}</tbody></table>
          </div>` : ''}
          ...
          <div class="section">
            ...
            <div class="analysis-box">
              ...
              <p>${finalScore >= 80 ? 'The assessment demonstrates strong alignment with learning objectives and cognitive levels.' :
                   finalScore >= 60 ? 'The assessment shows reasonable alignment with room for improvement in question design and distribution.' :
                   'Significant adjustments are required to align with expected standards and cognitive level distribution.'}</p>
            </div>
            <div class="analysis-box">
              ...
              <p>${matchPercentage}% of questions match their expected Bloom's taxonomy levels, with ${higherQuestions} questions at higher cognitive levels and ${lowerQuestions} below target. ${
                matchPercentage >= 80 ? 'This indicates strong cognitive level alignment across the assessment.' :
                matchPercentage >= 60 ? 'Consider reviewing questions that fall below expected cognitive levels.' :
                'A substantial revision of question design is recommended to improve alignment.'}</p>
            </div>
          </div>
  ...
  if (error) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-lg shadow-sm border border-gray-200 max-w-md w-full">
        <div className="flex items-center justify-center mb-4"><AlertCircle className="h-12 w-12 text-red-500" /></div>
        ...
        <button onClick={() => { const t = sessionStorage.getItem('accessToken'); if (t) fetchData(t); }}
  ...
  if (!data) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-lg shadow-sm border border-gray-200 max-w-md w-full">
        <div className="flex items-center justify-center mb-4"><AlertCircle className="h-12 w-12 text-gray-500" /></div>
  ...
  // ── CO-PO data (correct backend paths) ──
  // CO_PO_Map: stored by upload route at sequence[0].CO_PO_Map (mapping strengths 1/2/3)
  // PO_Coverage, CO_PO_Coverage, CO_PO_Penalty: stored by evaluate.js inside Collected Data[0]
  const coPOMap      = sequence[0]?.CO_PO_Map || {};
  const poCoverage   = collectedData?.PO_Coverage || {};
  const coPOCoverage = collectedData?.CO_PO_Coverage || {};
  const coPOPenalty  = collectedData?.CO_PO_Penalty ?? null;   // null = no PO mapping provided
  // PO Achievement = complement of the penalty (100 - penalty%)
  const poAchievement = coPOPenalty !== null ? parseFloat((100 - coPOPenalty).toFixed(2)) : null;

  // Derive all unique PO keys from CO_PO_Map (union across all COs)
  const coKeys = Object.keys(coPOMap);
  const poKeys = coKeys.length > 0
    ? [...new Set(coKeys.flatMap(co => Object.keys(coPOMap[co] || {})))]
    : [];
  ...
  const getScoreRemark = (score) => {
    if (score >= 80) return { label: 'Excellent', description: 'Strong alignment and balanced distribution. Minor refinements may enhance quality further.', color: 'text-green-600', bgColor: 'bg-green-50', borderColor: 'border-green-200' };
    if (score >= 60) return { label: 'Good', description: 'Reasonable alignment with some areas needing attention. Review under-represented modules/COs.', color: 'text-blue-600', bgColor: 'bg-blue-50', borderColor: 'border-blue-200' };
    if (score >= 40) return { label: 'Moderate', description: 'Significant improvements needed. Revise question cognitive levels and balance distribution.', color: 'text-yellow-600', bgColor: 'bg-yellow-50', borderColor: 'border-yellow-200' };
    return { label: 'Poor', description: 'Comprehensive restructuring required. Major misalignment in cognitive levels and/or distribution.', color: 'text-red-600', bgColor: 'bg-red-50', borderColor: 'border-red-200' };
  };

  const scoreRemark = getScoreRemark(finalScore);

  // Helper: color class based on coverage %
  const coverageColor = (val) =>
    val >= 80 ? 'text-green-600 font-bold' :
    val >= 60 ? 'text-blue-600 font-semibold' :
    val >= 40 ? 'text-amber-600 font-semibold' :
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-6 mb-6 overflow-hidden relative">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 mt-1">
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              <button onClick={openAppendix} className="flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 py-2.5 px-5 rounded-xl border border-gray-200 transition-colors text-sm font-medium">
                <BookOpen className="h-4 w-4" />View Appendix
              </button>
              <button onClick={() => setShowVisualization(true)} disabled={isDownloading} className="flex items-center gap-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 py-2.5 px-5 rounded-xl border border-indigo-200 transition-colors text-sm font-medium">
                <Download className="h-4 w-4" />Visualize Score
              </button>
              <button onClick={downloadCertificate} disabled={isDownloading} className="flex items-center gap-2 bg-amber-50 hover:bg-amber-100 text-amber-700 py-2.5 px-5 rounded-xl border border-amber-200 transition-colors text-sm font-medium">
                <Download className="h-4 w-4" />{isDownloading ? 'Generating…' : 'Certificate'}
              </button>
              <button onClick={downloadPDF} disabled={isDownloading} className="flex items-center gap-2 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white py-2.5 px-5 rounded-xl shadow-md shadow-blue-500/20 transition-all text-sm font-medium">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-8 mb-6 relative overflow-hidden">
          ...
          <div className="text-center">
            ...
            <div className="flex justify-center mb-6"><Gauge value={finalScore} size={240} /></div>
        ...
        {/* ── CO-PO Mapping (strength table: 1/2/3) — only if PO data exists ── */}
        {poKeys.length > 0 && (
          <Card title="CO-PO Mapping" accent="purple">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b-2 border-gray-100">
                    <Th>CO</Th>
                    {poKeys.map(po => <Th key={po} center>{po}</Th>)}
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {coKeys.map(co => {
                    ...
                    return (
                      <tr key={co} className="hover:bg-slate-50 transition-colors">
                        <Td>
                          <span className="px-2 py-0.5 bg-purple-50 border border-purple-100 text-purple-700 rounded-md font-semibold text-xs">{co}</span>
                        </Td>
                        {poKeys.map(po => {
                          const val = mapping[po];
                          const display = (!val || val === '') ? '–' : val;
                          const color =
                            ...
                            val === 2 ? 'text-blue-600 font-semibold' :
                            val === 1 ? 'text-amber-600 font-semibold' :
                                        'text-gray-300';
                          return (
                            <td key={po} className={`px-4 py-3 text-center text-sm ${color}`}>{display}</td>
                          );
                        })}
                      </tr>
                    );
              ...
              <div className="flex gap-4 mt-3 text-xs text-gray-500">
                <span><span className="text-green-600 font-bold">3</span> = High</span>
                <span><span className="text-blue-600 font-semibold">2</span> = Medium</span>
                <span><span className="text-amber-600 font-semibold">1</span> = Low</span>
                <span><span className="text-gray-300">–</span> = No relation</span>
              </div>
            </div>
          </Card>
        )}

        {/* ── CO-PO Analysis (penalty, PO achievement, PO coverage, CO-PO coverage) ── */}
        {poKeys.length > 0 && (
          <Card title="CO-PO Analysis" accent="purple">

            {/* Summary: penalty vs achievement */}
            {poAchievement !== null && (
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="rounded-xl border border-red-100 bg-red-50/50 p-5 text-center">
                  <div className="text-xs text-gray-400 uppercase tracking-wider mb-1">CO-PO Penalty</div>
                  <div className="text-3xl font-bold text-red-500">{coPOPenalty.toFixed(2)}%</div>
                  <div className="text-xs text-gray-400 mt-1">% of PO coverage not achieved</div>
                </div>
                <div className={`rounded-xl border p-5 text-center ${
                  poAchievement >= 80 ? 'border-green-100 bg-green-50/50' :
                  poAchievement >= 60 ? 'border-blue-100 bg-blue-50/50' :
                  poAchievement >= 40 ? 'border-amber-100 bg-amber-50/50' :
                                        'border-red-100 bg-red-50/50'
                }`}>
                  <div className="text-xs text-gray-400 uppercase tracking-wider mb-1">PO Achievement</div>
                  <div className={`text-3xl font-bold ${coverageColor(poAchievement)}`}>{poAchievement.toFixed(2)}%</div>
                  <div className="text-xs text-gray-400 mt-1">complement of penalty (100 − penalty)</div>
                </div>
              </div>
            )}

            {/* PO Coverage table — one row showing each PO's coverage % */}
            <h3 className="text-sm font-semibold text-gray-700 mb-3 uppercase tracking-wider">PO Coverage</h3>
                ...
                <tbody>
                  <tr>
                    {poKeys.map(po => {
                      const val = poCoverage[po] ?? 0;
                      return (
                        <td key={po} className={`px-4 py-3 text-center text-sm ${coverageColor(val)}`}>
                          {val.toFixed(1)}%
                        </td>
                      );
                    })}
            ...
            {/* CO-PO Coverage table — per CO-PO pair coverage % */}
            <h3 className="text-sm font-semibold text-gray-700 mb-3 uppercase tracking-wider">CO-PO Mapping Coverage</h3>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b-2 border-gray-100">
                  ...
                  {coKeys.map(co => {
                    ...
                    return (
                      <tr key={co} className="hover:bg-slate-50 transition-colors">
                        ...
                        {poKeys.map(po => {
                          const val = coverageRow[po];
                          // null = no mapping defined for this CO-PO pair
                          if (val === null || val === undefined) {
                            return <td key={po} className="px-4 py-3 text-center text-sm text-gray-300">–</td>;
                          }
                          return (
                            <td key={po} className={`px-4 py-3 text-center text-sm ${coverageColor(val)}`}>
                              {val.toFixed(1)}%
              ...
              <p className="text-xs text-gray-400 mt-3">
                Coverage % = how much of each CO-PO pair's maximum potential is actually achieved.
                &nbsp;<span className="text-gray-300">–</span> = no mapping defined (excluded from calculation).
              </p>
            </div>
          </Card>
        )}
        ...
        <Card title="Module Distribution" accent="purple">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b-2 border-gray-100"><Th center>Module</Th><Th center>Hours</Th></tr></thead>
              <tbody className="divide-y divide-gray-50">
        ...
        <Card title="Bloom's Taxonomy Mapping" accent="teal">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b-2 border-gray-100"><Th>Cognitive Level</Th><Th center>Value</Th></tr></thead>
              <tbody className="divide-y divide-gray-50">
                {Object.keys(blommLevelMap).map(level => (
                  <tr key={level} className="hover:bg-slate-50 transition-colors">
                    <Td>{level}</Td><Td center>{blommLevelMap[level]}</Td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Card title="Module Coverage Analysis" accent="teal">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b-2 border-gray-100">
                <Th center>Module</Th><Th center>Expected</Th><Th center>Actual</Th><Th center>Variance</Th>
              </tr></thead>
              <tbody className="divide-y divide-gray-50">
              ...
              <tbody className="divide-y divide-gray-50">
                {questionRecommendations.map((rec, index) => (
                  <tr key={index} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className={`px-4 py-3 text-center text-sm font-bold ${rec.qScore === 1 ? 'text-green-600' : rec.qScore === 2 ? 'text-blue-600' : 'text-red-500'}`}>{rec.qScore}</td>
                    <td className={`px-4 py-3 text-center text-xs font-semibold ${rec.remark === 'Matches Expected Blooms Level' ? 'text-green-600' : rec.remark === 'Higher than Expected Blooms Level' ? 'text-blue-600' : 'text-red-500'}`}>{rec.remark || 'No remarks'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead><tr className="border-b-2 border-amber-100 bg-amber-50/50">
                        <Th center>CO</Th><Th center>Actual</Th><Th center>Expected</Th><Th center>Variance</Th><Th>Recommendation</Th>
                      </tr></thead>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead><tr className="border-b-2 border-blue-100 bg-blue-50/50">
                        <Th center>Module</Th><Th center>Actual</Th><Th center>Expected</Th><Th center>Variance</Th><Th>Recommendation</Th>
                      </tr></thead>
          ...
          <div className="space-y-4">
            <div className="border-l-4 border-blue-400 bg-blue-50/60 rounded-r-xl p-4">
              ...
              <p className="text-sm text-gray-600">
                {finalScore >= 80 ? 'The assessment demonstrates strong alignment with learning objectives and cognitive levels.' :
                 finalScore >= 60 ? 'The assessment shows reasonable alignment with room for improvement in question design and distribution.' :
            ...
            {/* PO Achievement summary in performance section — only if PO data available */}
            {poAchievement !== null && (
              <div className="border-l-4 border-indigo-400 bg-indigo-50/60 rounded-r-xl p-4">
                <h3 className="font-semibold text-gray-800 mb-1 text-sm">PO Achievement</h3>
                <p className="text-sm text-gray-600">
                  Overall PO achievement is <span className={`font-bold ${coverageColor(poAchievement)}`}>{poAchievement.toFixed(2)}%</span> (CO-PO penalty: {coPOPenalty.toFixed(2)}%).
                  {poAchievement >= 80 ? ' Strong PO coverage across all mapped outcomes.' :
                   poAchievement >= 60 ? ' Moderate PO coverage — some outcomes need more question coverage.' :
                   ' Low PO coverage — review CO-PO mappings and increase question diversity across mapped outcomes.'}
                </p>
              </div>
            )}
          </div>
