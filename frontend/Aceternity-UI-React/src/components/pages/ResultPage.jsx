...
import VisualizationModal, { Gauge } from './VisualizationModal';
    ...
    try {
      ...
      const questionRows = questionRecommendations.map((rec, i) => {
        ...
        <tr>
          ...
          <td class="text-center ${rec.qScore === 0 ? 'status-match' : rec.qScore > 0 ? 'status-higher' : 'status-lower'}">${rec.qScore}</td>
              ...
              <tbody className="divide-y divide-gray-50">
                {questionRecommendations.map((rec, index) => (
                  <tr key={index} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className={`px-4 py-3 text-center text-sm font-bold ${rec.qScore === 0 ? 'text-green-600' : rec.qScore > 0 ? 'text-blue-600' : 'text-red-500'}`}>{rec.qScore}</td>
